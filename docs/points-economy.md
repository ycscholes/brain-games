...
游戏大闯关每次随机抽取 3 个不同的真实单项游戏，排除 `head-count` 这类兼容跳转页和 `game-gauntlet` 自身。子游戏在闯关模式下只回传本局原始成绩、积分难度、模式和按单项规则换算出的应得宠物积分，不写单项训练记录、不刷新单项最高分、不单独发放宠物积分。闯关结束后只写入一条 `gameId: "game-gauntlet"` 训练记录，`score` 与 `awardedPoints` 都等于三局子游戏应得积分之和；该汇总奖励继续通过 `getAwardedPoints()` 与 `addPointsToPet()` 共享管线发放，但使用独立 `maxPoints = 汇总分` 的奖励策略，避免默认单局 40/60 上限二次截断。游戏大闯关不展示最高分。
