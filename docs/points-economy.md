...
| **24 点 (twenty-four)** | 2.0x | 20-40 | 30-60 | 每题 2 游戏分，单题耗时较高 |
...
24 点每道未使用提示且正确完成的题目获得 2 游戏分，四张牌的数字范围在普通和困难模式下均为 `0..10`。宠物积分继续通过共享管线按 `2.0x` 转换：每题基础获得 4 宠物积分；普通模式封顶 40，困难模式应用 1.5 倍后封顶 60。
...
- ✅ 24 点游戏逻辑测试覆盖 `0..10` 数字边界、可解题目生成、每题 2 分和表达式求值
