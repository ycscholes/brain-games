...
速算挑战不再让玩家单独选择积分难度，而是由所选训练内容自动决定：`10以内加减`、`20以内进退位`、`百以内与口诀` 按普通积分计算；`万以内加减乘除`、`多位数乘除`、`整数四则混合` 按困难积分计算。训练记录仍使用 `gameId: "mental-math"`，`mode` 记录为 `限时/闯关模式:阶段ID`，例如 `timed:G3`。
