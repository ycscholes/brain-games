...
| **24 点 (twenty-four)** | 2.0x | 12-40 | 18-60 | 初始每题 2 分，每答对 3 题后续每题加 1 分 |
...
24 点每局初始每题 2 游戏分，每累计答对 3 题，后续每题分值增加 1，即前 9 题依次为 `2/2/2/3/3/3/4/4/4` 分。错误、跳过和提示题不推进计分阶梯；四张牌的数字范围在普通和困难模式下均为 `1..10`。宠物积分继续通过共享管线按 `2.0x` 转换，普通模式封顶 40，困难模式应用 1.5 倍后封顶 60。
...
- ✅ 24 点游戏逻辑测试覆盖 `1..10` 数字边界、可解题目生成、阶梯计分序列和表达式求值
