...
用户也可以上传一张单只宠物图片生成 AI 自定义宠物。服务端冻结 300 积分，生成 `idle/feed/cuddle/hungry` 四张私有图片，并为每次生成提供一次免费整套重做。确认领养后才结算积分；最终技术失败、预览前取消或主动放弃会退回积分。暂时不限制每个用户的历史生成次数，但同一用户同一时间只能存在一个活动生成任务。自定义宠物可永久删除，删除后仍可重新发起新的自定义宠物生成。
