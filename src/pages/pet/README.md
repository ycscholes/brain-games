...
用户也可以上传一张单只宠物图片生成 AI 自定义宠物。服务端冻结 300 积分，生成 `idle/feed/cuddle/hungry` 四张私有图片，首次完整预览占用每用户一次的生成资格，并提供一次免费整套重做。确认领养后才结算积分；最终技术失败或预览前取消会退回积分。自定义宠物可永久删除，但生成资格不恢复。
