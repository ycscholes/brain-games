...
function buildMoodSheetPrompt({ includeReferenceRoles = false }) {
  ...
  return [
    includeReferenceRoles
      ? "参考图规则：第 1 张用户上传图是唯一宠物身份和外观来源，最高优先级；四格必须以第 1 张的物种、脸型、耳朵、眼睛、嘴吻、身体比例、毛色、花纹分布、尾巴和原有配饰为准；第 2 张四状态参考图仅参考 idle、feed、cuddle、hungry 的姿态、构图、表情和水彩画风；禁止参考第 2 张的物种、脸型、耳朵、眼睛、嘴部、体型、毛色、花纹、尾巴等任何外观特征；两张图冲突时始终以第 1 张为准，角色身份一致性优先于姿态一致性"
      : "",
    "单只第 1 张参考图中的宠物四状态角色设定图，2x2 四宫格",
    "左上 idle：自然站立或坐着，平静看向前方",
    "右上 feed：参考第 2 张右上状态的开心表情和姿态，仅允许嘴部轻微动作",
    ...
    REFERENCE_VISUAL_TRAITS_PROMPT,
    EXTRA_CONTENT_NEGATIVE_PROMPT,
    "儿童绘本水彩风格，主体居中，每格四周保留安全边距，只调整姿态和表情",
